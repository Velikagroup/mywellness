...
function LayoutContent({ children }) {
  ...
  React.useEffect(() => {
    ...
    window.fbq('init', '1636022610082279');
    window.fbq('track', 'PageView');
  }, []);

  // Track Meta PageView on route change
  React.useEffect(() => {
    if (window.fbq) {
      window.fbq('track', 'PageView');
    }
    ...
    <div className={`min-h-screen ${!location.pathname.toLowerCase().includes('quiz') ? 'animated-gradient-bg' : 'bg-white'}`}>
      ...
      {location.pathname !== createPageUrl('Video') && !location.pathname.toLowerCase().includes('quiz') && (
        <div
          ...
          >
            <div className="water-glass-effect rounded-full px-4 py-2 flex items-center gap-2">
            {showBackButton ? (
              <>
                <button
                  onClick={() => navigate(-1)}
                  className="flex items-center justify-center px-4 py-2 -ml-1 text-gray-700 hover:text-gray-900 transition-colors"
                >
