...
export default function Checkout() {
  ...
  const planDescriptions = {
    base: 'Nutrizionale completo con dashboard scientifica',
    pro: 'Nutrizionale + Allenamento + Analisi AI',
    premium: 'Accesso completo a tutte le funzionalità'
  };
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (!checkoutTracked && currentUser && currentUser.email) {
          try {
            ...
            // Still set tracked to avoid retry loops
            setCheckoutTracked(true);
          }
        }
        ...
        if (planParam && ['base', 'pro', 'premium'].includes(planParam)) {
          setSelectedPlan(planParam);
        }
        
        if (billingParam && ['monthly', 'yearly'].includes(billingParam)) {
          setSelectedBillingPeriod(billingParam);
        }
        ...
        const checkoutUrl = window.location.origin + createPageUrl('Checkout') + location.search;
    ...
    checkAuth();
  }, [navigate, location, checkoutTracked, selectedPlan]);
    ...
    const initializePaymentRequest = async () => {
      try {
        const pr = stripe.paymentRequest({
          ...
          currency: 'eur',
          total: {
            label: `MyWellness ${planNames[selectedPlan]}`,
  ...
  }, [stripe, user, selectedPlan, selectedBillingPeriod, appliedCoupon]);
      ...
      paymentRequest.on('paymentmethod', async (ev) => {
        try {
          ...
          const payload = {
            ...
            skipTrial: true, // Always skip trial - direct payment
            billingInfo: {
  ...
  const monthlyEquivalent = selectedBillingPeriod === 'yearly' ? (finalPrice / 12).toFixed(2) : finalPrice.toFixed(2);
        ...
        <Card className="max-w-2xl w-full water-glass-effect border-gray-200/30 shadow-2xl rounded-2xl overflow-hidden">
          <CardHeader className="text-center pb-6 pt-8">
            ...
            <p className="text-gray-600 text-base">
              Piano {planNames[selectedPlan]} - {planDescriptions[selectedPlan]}
            </p>
              ...
              <>
                ...
                <div className="bg-gray-50/50 rounded-xl p-4 space-y-2">
                  ...
                  <p className="text-xs text-gray-500 text-center pt-2">
                    Piano {planNames[selectedPlan]} - {selectedBillingPeriod === 'monthly' ? 'Mensile' : 'Annuale'} 
                    {selectedBillingPeriod === 'yearly' && ` (€${monthlyEquivalent}/mese)`}
                  </p>
                </div>
                ...
                <p className="text-xs text-center text-gray-500 mt-4 px-2">
                  Accesso immediato a tutte le funzionalità del piano {planNames[selectedPlan]}. Cancella in qualsiasi momento.
                </p>
              </>
            )}
          </CardContent>
