...
export default function AdminCoupons() {
  ...
  const couponStats = useMemo(() => {
    ...
    coupons.forEach(coupon => {
      ...
      console.log(`🎫 Coupon ${coupon.code} - utenti trovati:`, usersWithThisCoupon.map(u => u.email));
    ...
    allPaidTx.filter(tx => allCouponUserIds.has(tx.user_id)).forEach(tx => {
      ...
      if (monthlyStats[key]) {
        ...
        monthlyStats[key].revenue += tx.amount;
      }
    });
  ...
  }, [coupons, transactions, users, selectedPeriod, selectedYear, selectedMonth]);
