...
export default function LiquidGlassNav({ navItems }) {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 w-full px-4 md:w-auto">
      <div
        ref={containerRef}
        className="water-glass-effect rounded-3xl relative py-3 px-2 w-full md:px-6 md:min-w-[500px]"
        ...
        <div className={`relative flex items-center z-10 ${isMobile ? 'justify-between w-full' : 'justify-center gap-1'}`}>
          ...
          {adminItems.length > 0 && !isMobile && adminItems.map((item) => (
            <Link
              key={item.name}
              to={createPageUrl(item.path)}
              className="flex flex-col items-center gap-1 p-2 md:p-3 rounded-xl transition-colors text-gray-400 hover:text-[#26847F] pointer-events-auto min-w-max"
              style={{ cursor: 'pointer' }}
              >
              <item.icon className="w-5 h-5 md:w-6 md:h-6" />
              <span className="text-xs font-medium text-center leading-tight">{item.name}</span>
