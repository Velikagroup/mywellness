...
export default function MealDetailModal({ meal, onClose, onMealUpdate }) {
  ...
  const generateAndSetImage = useCallback(async (mealToUpdate, replacementInfo = null) => {
    ...
    try {
      ...
      setCurrentMeal(prevMeal => ({...prevMeal, image_url: imageResponse.url})); 
      onMealUpdate(finalUpdatedMeal);
    } catch (error) {
      console.error("Error generating image:", error);
      setCurrentMeal(prevMeal => ({ ...prevMeal, image_url: null }));
  ...
  const handleReplaceIngredient = async (ingredientToReplace) => {
    ...
    try {
        ...
        const newIngredientFromLLM = await base44.integrations.Core.InvokeLLM({
        ...
        const newIngredientsWithActiveStatus = currentMeal.ingredients.map(ing =>
            ing.name === ingredientToReplace.name
                ? { ...newIngredientFromLLM, is_active: true }
                : ing
        );
        ...
        const updatedMealFromDB = await base44.entities.MealPlan.update(currentMeal.id, mealDataForDBUpdate);
        ...
        onMealUpdate(finalMealForUI);
        setCurrentMeal(finalMealForUI);
        ...
        alert("Errore nella sostituzione dell'ingrediente. Riprova.");
        setCurrentMeal({
          ...meal,
          ingredients: meal.ingredients.map(ing => ({ ...ing, is_active: true }))
        });
