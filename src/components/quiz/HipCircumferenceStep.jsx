...
export default function HipCircumferenceStep({ data, onDataChange, translations }) {
  ...
  useEffect(() => {
    if (data.hip_circumference) {
      if (unit === 'cm') {
        setDisplayValue(data.hip_circumference.toString());
      } else {
        setDisplayValue((data.hip_circumference / 2.54).toFixed(1));
      }
    }
  }, [data.hip_circumference, unit]);
  ...
  const handleChange = (e) => {
    ...
    if (unit === 'cm') {
      onDataChange({ hip_circumference: parseFloat(value) || '' });
    } else {
      const cm = parseFloat(value) * 2.54;
      onDataChange({ hip_circumference: parseFloat(cm.toFixed(1)) || '' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="max-w-md mx-auto">
        ...
        <p className="text-sm text-gray-500 mt-2 text-center">{t.quizHipFemaleOnly || "Necessaria per calcolare la massa grassa (solo donne)"}</p>
      </div>
    </div>
  );
}